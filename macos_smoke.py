...
def run():
    ...
    with tempfile.TemporaryDirectory(prefix='yingxu-macos-smoke-') as temporary:
        ...
        try:
            ...
            checks.append('Every offline canvas asset and font matches the bundled manifest')
            ...
            assert Path(changed['path']).read_bytes()==original
            checks.append('Markdown file links resolve without rewriting the source note')
            ...
            checks.append('Maintenance preview and empty cleanup retain original documents')
