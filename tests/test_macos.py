...
class MacAdaptersTests(unittest.TestCase):
    def test_preview_and_close_drains_all_workers_in_order_once(self):
        self.assertEqual(PREVIEW, '0.4.9-mac.1')
        app=Application.__new__(Application)
        app._close_lock=threading.Lock();app._closed=False
        events=[]
        app.jobs=Mock();app.thumbnails=Mock();app.context=Mock()
        ...
        app.close();app.close()
        self.assertEqual(events,[('jobs',{'wait':True,'cancel_futures':False}),('thumbnails',{'wait':True,'cancel_futures':False}),('context',{})])
