...
test('capture settings offer annotation by default and persist quick or annotate mode',async()=>{
  for(const mode of ['quick','annotate']){const s=setup();assert.equal(s.preference('capture_mode'),'annotate');await s.settingsDialog();assert.match(s.dialogs[0].body,/name="capture_mode"/);assert.match(s.dialogs[0].body,/<option value="annotate" selected>/);assert.match(s.dialogs[0].body,/标注后确认/);await s.dialogs[0].onSubmit({capture_mode:mode,capture_hotkey:'Ctrl+Alt+Shift+S',default_view:'grid',default_sort:'updated'});assert.equal(s.calls[1].options.body.capture_mode,mode);assert.equal(s.state.bootstrap.settings.capture_mode,mode);assert.equal(s.messages[0].action,'settings-changed');}
});
test('external documents honor editable capability without creating project records',async()=>{const s=setup();await s.openExternal('a'.repeat(32));assert.equal(s.state.tabs[0].source,'external');assert.equal(s.state.tabs[0].mode,'edit');assert.equal(s.state.tabs[0].content.editable,true);assert.equal(s.state.tabs[0].dirty,false);assert.equal(s.calls.length,1);assert.equal(s.calls[0].url,'/api/external/'+'a'.repeat(32));await s.openExternal('a'.repeat(32));assert.equal(s.state.tabs.length,1);});
