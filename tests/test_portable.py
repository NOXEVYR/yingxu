...
class PortablePathsTests(unittest.TestCase):
    ...
    def test_defaults_are_per_user_and_do_not_create_directories(self):
        with patch('yingxu.paths.documents_directory', return_value=self.root / '文档'), patch('yingxu.paths.Path.home', return_value=self.root):
            expected = self.root / 'Library' / 'Application Support' / 'YingXu' if sys.platform == 'darwin' else self.root / 'Local' / 'YingXu'
