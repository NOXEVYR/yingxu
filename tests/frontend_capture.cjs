...
function appSetup(){const nodes=new Map(),ctx=vm.createContext({console,setTimeout,clearTimeout,document:{querySelector:k=>{if(!nodes.has(k))nodes.set(k,{value:'',open:false});return nodes.get(k);}},window:{},localStorage:{getItem:()=>null,setItem(){}}});vm.runInContext(fs.readFileSync(path.join(__dirname,'../frontend/markdown-preview.js'),'utf8'),ctx);vm.runInContext(fs.readFileSync(path.join(__dirname,'../frontend/app.js'),'utf8').replace(/boot\(\);\s*$/,'')+'\nglobalThis.app={state,captureTarget,insertCapture,inlineMarkdown,markdownImageURL};',ctx);return ctx.app;}
