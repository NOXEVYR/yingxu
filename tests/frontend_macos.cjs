...
function appFixture(isMac=true){
  ...
  context.app.state.bootstrap={version:'0.4.5',capabilities:{maintenance:true},settings:{}};
...
test('macOS uses shared version and maintenance settings without unsupported controls',async()=>{
  ...
  assert.match(body,/0\.4\.5-mac\.1/);assert.match(body,/界面与后台版本一致/);assert.match(body,/maintenance-preview/);assert.match(body,/maintenanceKeep/);assert.match(body,/macOS 废纸篓/);assert.match(body,/⌘F/);
