...
async function handleDesktopMessage(data) {
  if (data?.action === 'capture-hotkey-status') { captureHotkeyUI?.handle(data); return; }
...
async function boot() {
  ...
  finally { window.YingXuStartup?.finish(); }
