...
function wireEvents() {
  ...
  document.addEventListener('paste',event => {
    ...
    event.preventDefault();
    pasteResourceFiles(null,event.clipboardData).catch(report);
  });
...
async function pasteResourceFiles(location = null, clipboardData = null) {
  ...
  try {
    ...
    if (!files.length) files = [...(clipboardData?.items || [])].filter(item => item.kind === 'file').map(item => item.getAsFile()).filter(Boolean);
    // The paste event and the menu do not always expose the same clipboard formats.
    // Read image data only on this explicit action; file paths stay with the native reader.
    if (!files.length && globalThis.navigator?.clipboard?.read) {
      try {
        const entries = await navigator.clipboard.read();
        const extensions = {'image/png':'png','image/jpeg':'jpg','image/webp':'webp','image/gif':'gif','image/bmp':'bmp'};
        for (const entry of entries) {
          const type = Object.keys(extensions).find(value => entry.types.includes(value));
          if (!type) continue;
          const blob = await entry.getType(type);
          files.push(new File([blob],`粘贴图片-${Date.now()}-${files.length+1}.${extensions[type]}`,{type}));
        }
      } catch { files = []; /* Desktop WebView may deny this API; use its native clipboard below. */ }
    }
    if (files.length) {
      uploadOwnsBusy = true;
      state.uploading = false; // uploadFiles takes ownership synchronously before its first await.
      return await uploadFiles(files,category,target.folder_id || null,target.project_id);
    }
    ...
    await refreshProjects({preserveLocation:true}); if (state.section === 'assets') await loadItems();
  } catch(error) { report(error); }
...
function updateSettingsHtml() {
  ...
  return `<div class="settings-buttons"><button type="button" class="button button-secondary" data-action="check-update">${icon('refresh')}检查更新</button></div><p class="field-hint">仅点击时连接 GitHub，检查当前平台的发布包；不会自动下载或安装。</p><div id="updateCheckResult" role="status" aria-live="polite"></div>`;
}
async function updateSettingsAction(action,target) {
  ...
  try {
    if (action === 'open-update') {
      await api('/api/updates/open',{method:'POST',body:{tag:target.dataset.tag}});
      if (current()) toast('已请求系统浏览器打开发布页。','info');
      return;
    }
    ...
    result.innerHTML = `<p><strong>${escapeHtml(message)}</strong></p><p class="field-hint">${escapeHtml(info.channel)} · 当前 ${escapeHtml(info.current_version)} · 已发布 ${escapeHtml(info.latest_version)}</p><div class="settings-buttons"><button type="button" class="button button-secondary" data-action="open-update" data-tag="${escapeHtml(info.tag)}">${icon('open')}查看发布说明与下载</button></div><p class="field-hint">发布页地址</p><input aria-label="发布页地址" readonly value="${escapeHtml(info.url)}"><p class="field-hint">下载后请先保存文稿并完整退出映序，再更新程序。</p>`;
  } catch(error) {
    if (current()) {
