...
- 本仓库正在同步 macOS **0.4.4-mac.1** 试用构建，目标为 macOS 14+、Apple Silicon（arm64）。新版下载尚未开放；构建与验证完成前，上述 0.4.3-mac.1 入口继续有效。试用版未公证，尚未完成人工中文输入法与不同 DPI 显示环境验收，不将自动化构建通过表述为完整 Mac 使用验收。
