...
**Windows 0.4.20 源码更新：修复项目迁移被选择窗口误阻塞和进度误显示。** 文件夹选择只读取路径，不再被计为文件写入；真正的写入和未保存内容仍阻止迁移。启动未被接受时明确提示“迁移尚未开始”并收起进度；连接状态不明时保留任务重连与编辑保护。复制校验、原目录备份及断电恢复规则不变，无新增依赖。完整包下载仍以下方已发布附件为准。
