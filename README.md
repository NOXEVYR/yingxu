...
**Windows 0.4.19 源码更新：修复兼容目录下的文件打开，并加入按文件增量更新。** 双击本机目录联接中的 Markdown、文本等文件时，先解析到真实文件位置，修复误报“拖出路径包含联接或符号链接”。只读阅览不改写原文，转入工作台编辑使用同一真实文件。设置内可按需检查、下载变化的程序文件并在退出后安装；无新增运行依赖、模型或启动联网。完整包状态以下方已公开附件为准。[增量更新说明](docs/incremental-updates.md)
...
**0.4.17 已收录功能：删除整个项目分类。** 在项目库选中自定义分类，点击“删除分类及全部内容”，可一次移除子分类和项目。默认勾选同时回收磁盘项目文件夹，下一步会列出实际路径并再次确认；外部引用原文件保留。取消磁盘处理时，项目仍保留在映序回收站，恢复后进入“未分类”。已回收的磁盘文件请从系统回收站还原。

**0.4.17 已收录功能：Windows 快速阅览。** 在资源管理器中以映序打开 Markdown、TXT、JSON、CSV 或字幕文本时，直接显示可调整大小、最大化和缩放的独立只读窗口；无需启动项目后台。右上角“在映序中打开”进入主端编辑原文件。重复打开同一文件复用窗口，关闭最后一个阅览窗口会退出仅阅览的进程。
