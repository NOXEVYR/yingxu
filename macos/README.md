...
- 选择文件夹导入项目，复制到设置的项目存放位置，按当前项目库分类登记，识别文本、角色、场景等目录后打开副本；原目录保留。
- 外部文档默认独立阅读与编辑，右上角可恢复完整工作台；切换复用编辑器，保留草稿和撤销记录。
- 中文组词期间保留后续外部打开请求，避免打断输入；独立文档的资源操作不会作用于隐藏项目。
- 无新增运行依赖、启动扫描或常驻监控。最终 ZIP 已完成原生、WKWebView、逐项校验和上传后完整下载回检。
- Windows 原生拖放桥接与全局截图不属于 macOS 功能；人工输入法、权限及长期稳定性仍需试用反馈。

[macOS 0.4.15-mac.1 历史包与校验](https://github.com/turnsolesama/yingxu/releases/tag/yingxu-v0.4.15-mac.1)保留。
