...
TAG='yingxu-v0.4.4-mac.1'
NAME='YingXu-v0.4.4-mac.1-macOS-arm64'
...
def main():
    ...
    names=[NAME+'.zip',NAME+'-manifest.json',NAME+'-SHA256.txt','YingXu-v0.4.4-mac.1-verification.json']
    ...
    notes='''映序 0.4.4 macOS 试用版（0.4.4-mac.1），适用于 macOS 14+ 的 Apple Silicon / M 系列芯片。

## 本次更新

- 同步 0.4.4 文档内查找，Command+F 根据焦点搜索正文或当前资源，Command+K 保留全局搜索。
...
[Windows 0.4.4 完整包](https://github.com/turnsolesama/portfolio/releases/tag/yingxu-v0.4.4) · [完整更新记录](https://github.com/turnsolesama/yingxu#readme) · [Mac 安装说明](https://github.com/turnsolesama/yingxu/blob/main/macos/README.md)
'''
    with tempfile.TemporaryDirectory(prefix='yingxu-release-publish-') as temporary:
        ...
        if existing.returncode:
            gh('release','create',TAG,*[str(artifacts/n) for n in names],
               '--target',commit,'--title','映序 0.4.4 · macOS M 系列试用版',
        ...
        portal.write_text('''映序 0.4.4 下载入口。Windows 正式包和 macOS 试用包分别选择，历史版本保留。

## 当前下载

- [Windows x64 0.4.4 完整包](https://github.com/turnsolesama/portfolio/releases/download/yingxu-v0.4.4/YingXu-v0.4.4-Windows-x64.zip)
- [macOS M 系列 0.4.4-mac.1 试用版与校验](https://github.com/turnsolesama/yingxu/releases/tag/yingxu-v0.4.4-mac.1)

## 0.4.4 更新

补齐文档内查找、Word 跨页定位与 Markdown 文件链接；修复画板撤销、本地字体及后台刷新；整理右键菜单，加入版本和容量预览。详情见[更新记录](https://github.com/turnsolesama/yingxu#readme)。

Mac 包适用于 macOS 14+ Apple Silicon，未公证，暂不支持 Intel、全局截图或菜单栏常驻。各平台校验和与具体测试范围见对应发布页。Windows 包继续使用原仓库已验证附件，不重复构建。下方 Source code 是源码，不是安装包。
...
''',encoding='utf-8')
        gh('release','edit','downloads-2026-09-12','--title','映序 YingXu 0.4.4 · Windows 与 macOS 下载','--notes-file',str(portal))
