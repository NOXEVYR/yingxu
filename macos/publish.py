...
- 外部文档默认使用独立阅读/编辑视图，右上角可展开完整工作台；布局切换复用编辑器与工具栏，保留未保存内容和撤销记录。
- 独立文档中的 Delete、资源粘贴与标题栏拖放不会误操作隐藏项目；中文组词期间的外部打开请求会保留到输入完成。
    ...
    with tempfile.TemporaryDirectory(prefix='yingxu-release-publish-') as temporary:
        ...
        if existing.returncode:
            gh('release','create',TAG,*[str(artifacts/n) for n in names],
               '--target',commit,'--title','映序 0.4.16 · 文件夹导入与独立文档 · macOS M 系列试用版',
